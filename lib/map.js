...
export function mapLlmContextResponse(payload) {
  ...
  if (!('grounding' in payload) && !('sources' in payload)) {
    throw providerError('Brave LLM Context response carried neither a "grounding" nor a "sources" envelope; this is not an LLM Context payload');
  }
...
export function mapWebSearchResponse(payload) {
  ...
  if (!('web' in payload) && !('query' in payload)) {
    throw providerError('Brave web search response carried neither a "web" nor a "query" envelope; this is not a Web Search payload');
  }

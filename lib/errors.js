...
export function credentialMissingError(apiKeyEnv) {
  return new WebError(
    `Brave search has no API key for "${String(apiKeyEnv)}"; store it through the harness credentials service, export it in the environment that launches the harness, or put it in the launching directory's .env file. This plugin's configuration carries the reference only, never a key.`,

 ...
 * @param apiKeyEnv - the credential reference that resolved to nothing.
 * @returns the `WEB_PROVIDER_CREDENTIAL_MISSING` seam error.
 */
export function credentialMissingError(apiKeyEnv) {
  const reference = String(apiKeyEnv);
  return new WebError(
    `Brave search has no API key for "${reference}". Store the value for that reference through the harness credentials service (Settings > Plugins > Plugin configuration > Web search), export ${reference} in the environment that launches the harness, or put it in the harness home's .env file. The apiKeyEnv setting names the credential; it never holds the key itself, so a key typed into that field is not stored anywhere.`,
